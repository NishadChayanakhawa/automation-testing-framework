...
public class SOAPAPITestHelper {
	 ...
	 * 
	 * @since v1.0.0
	 * @param headersMap headers map as {@link java.util.Map
	 *                   Map&lt;String,String&gt;}
	...
	public static SOAPMessage formRequest(Map<String, String> headersMap, String soapAction, String payload)
		...
		if (headersMap != null) {
			...
			TestFactory.recordTestStep(
					String.format("Headers: <br><br><textarea>%s</textarea>", new JSONObject(headersMap).toString(1)));
		}
		// Attach SOAP Action
		if (soapAction != null) {
			headers.addHeader("SOAPAction", soapAction);
			TestFactory.recordTestStep(String.format("SOAPAction: %s", soapAction));
		}

		// Return Request Message
		// Create input stream from payload
		try (InputStream payloadStream = new ByteArrayInputStream(payload.getBytes())) {
			TestFactory.recordTestStep(String.format("Payload: <br><br><textarea>%s</textarea>", payload));
	...
	private static NamespaceContext getNamespaceContext(String namespacePrefix, String namespaceUri) {
		SOAPAPITestHelper.logger.debug("Creating Namespace context with prefix {} and uri {}", namespacePrefix,
				namespaceUri);
